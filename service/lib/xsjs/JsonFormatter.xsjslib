var formatter = class JsonFormatter{

    static escapeSpecialCharsText(input) {
        ...
        while (rs.next()) {
            for (var i=1; i<=colCount; i++) {
                ...
                switch(meta.getColumnType(i)) {
                    ...
                    case $.db.types.CHAR:
                        value += '"' + JsonFormatter.escapeSpecialChars(rs.getString(i))+'"';
                        break;
                    case $.db.types.NVARCHAR:
                    case $.db.types.NCHAR:
                    case $.db.types.SHORTTEXT:
                        value += '"' + JsonFormatter.escapeSpecialChars(rs.getNString(i))+'"';
                        break;
                    ...
                    case $.db.types.NCLOB:
                    case $.db.types.TEXT:
                        value += '"' + JsonFormatter.escapeSpecialChars(rs.getNClob(i))+'"';
                        break;
                    case $.db.types.CLOB:
                        value += '"'+ JsonFormatter.escapeSpecialChars(rs.getClob(i))+'"';
                        break;
                    case $.db.types.BLOB:
